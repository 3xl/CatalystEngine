...
async function start() {
  ...
  if (!loader) {
    state.value = 'error';
    error.value = `Demo sconosciuta: "${props.name}"`;
    return;
  }

  state.value = 'loading';
  try {
    // Import dinamico: Phaser (che usa window/document) viene caricato solo
    // qui, lato client, mai durante il render SSR.
...
// Cleanup obbligatorio: VitePress è una SPA, senza destroy il canvas e il
// game-loop resterebbero attivi navigando ad altre pagine.
...
<template>
  <figure class="demo">
    ...
    <div class="demo__frame" :style="{ minHeight: height + 'px' }">
      ...
      <div v-else-if="state === 'loading'" class="demo__overlay">Caricamento…</div>

      <div v-else-if="state === 'error'" class="demo__overlay demo__overlay--error">
        Impossibile avviare la demo.<br />
        <small>{{ error }}</small>
      </div>
    </div>

    <div v-if="state === 'running'" class="demo__bar">
      <button class="demo__restart" type="button" @click="restart">↻ Riavvia</button>
      <span class="demo__hint">Demo interattiva — usa i pulsanti nel canvas.</span>
    </div>
  </figure>
</template>
